...
import { clearErrorDashboard, fetchDashboard, setUseLocalStorage } from "@/store/jobsSlice";
...
export default function DashboardPage() {
  ...
  const { dashboard, loadingDashboard, errorDashboard } = useAppSelector(
    (state) => state.jobs
  );

  // Set data source based on authentication
  useEffect(() => {
   const isAuthenticated = !!session?.user;
    dispatch(setUseLocalStorage(!isAuthenticated));
  }, [session, dispatch]);
    ...
    return (
      <div className="mx-auto max-w-6xl">
        <Alert
          ...
          onDismiss={() => dispatch(clearErrorDashboard())}
        />
        <Button
          variant="secondary"
          className="mt-4"
  ...
  const isGuestMode = authStatus !== "loading" && !session?.user;
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-amber-800">
                You're currently using the app as a guest. Your data is stored in your browser's localStorage and will be lost if you clear your browser data.{" "}
